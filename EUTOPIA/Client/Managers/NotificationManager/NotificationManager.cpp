...
void NotificationManager::Render() {
    Vec2<float> windowSize =
        GI::getClientInstance()->guiData->windowSizeReal;  // D2D::getWindowSize();
    float textSize = 1.f;
    ...
    for(int i = 0; i < notifList.size(); i++) {
        ...
        if(notif->duration > 0.f) {
            Vec2<float> posTo =
                Vec2<float>(windowSize.x - offsetX - textWidth - (textPaddingX * 2.f), posY);
            ...
            if(notif->pos.x < windowSize.x) {
                // notif->pos.x = Math::lerp(notif->pos.x, windowSize.x + textPaddingX,
                // D2D::deltaTime * 8.f);
        ...
        float anim = (windowSize.x - notif->pos.x) /
                     (windowSize.x - (windowSize.x - offsetX - textWidth - (textPaddingX * 2.f)));
        if(anim > 1.f)
            anim = 1.f;

        Vec4<float> rectPos =
            Vec4<float>(notif->pos.x, notif->pos.y, notif->pos.x + textWidth + (textPaddingX * 2.f),
        ...
        D2D::drawGlowingText(textPos, message, UIColor(255, 255, 255, (int)(255 * anim)), textSize,
                             4.f);
